...
Original file is located at
    https://colab.research.google.com/drive/1vSyQzKnm5ZdYw2DlxhNA0aLzSZvNw4jy
"""
...
class ProjectRunner:
    ...
    def _merge(self, l1,l2,skip):
        ...
        while node1 is not None and node2 is not None:
          if node1.value == node2.value:
            merge.insert_at_end(node1.value)
            if node1.tfidf > node2.tfidf:
              merge.end_node.tfidf = node1.tfidf
            else:
              merge.end_node.tfidf = node2.tfidf
            node1 = node1.next
            node2 = node2.next
            count += 1
          elif node1.value<=node2.value:
            if skip == True:
              if node1.skip is not None and node1.skip.value <= node2.value:
                while node1.skip is not None and node1.skip.value <= node2.value:
                  node1 = node1.skip
                  count = count + 1
              else:
                node1 = node1.next
            ...
            if skip == True:
              if node2.skip is not None and node2.skip.value <= node1.value:
                while node2.skip is not None and node2.skip.value <= node1.value:
                  node2 = node2.skip
                  count += 1
              else:
                node2 = node2.next
    ...
    def _daat_and(self, input_array):
        ...
        # input_list = self.indexer.get_index()
        # print("hello")
        # print(type(input_array))
        postinglists = [self.indexer.inverted_index[t] for t in input_array]
        print(postinglists)
        postinglists.sort(key = lambda x: x.length)
        if len(postinglists)==0:
          return None, 0
        merged = copy.deepcopy(postinglists[0])
        count = 0
        for i in range(1, len(postinglists)):
          merged, comp = self._merge(merged, postinglists[i], False)
          count += comp
        
        merged_skip = copy.deepcopy(postinglists[0])
        count_skip = 0
        for i in range(1, len(postinglists)):
          merged_skip, comp = self._merge(merged_skip, postinglists[i], True)
          count_skip += comp

        result = merged.traverse_list()
        tfidf_list = merged.tfidf_values()


        result_sorted = [x[1] for x in sorted(zip(tfidf_list, result), key = lambda x: -1*x[0])]
        return result,count, result, count_skip, result_sorted, count, result_sorted, count_skip
    ...
    def _get_postings(self, term):
        ...
        inverse_index = self.indexer.get_index()
        for key in inverse_index.keys():
          if key == term:
            return LinkedList.traverse_list(inverse_index[term]), LinkedList.traverse_skips(inverse_index[term])
        # raise NotImplementedError
    ...
    def run_indexer(self, corpus):
        ...
        with open(corpus, 'r', encoding="utf-8") as fp:
            for line in tqdm(fp.readlines()):
                doc_id, _ = self.preprocessor.get_doc_id(line)
                tokenized_document, tfdict = self.preprocessor.tokenizer(line, doc_id)
                self.indexer.generate_inverted_index(doc_id, tokenized_document)
        self.indexer.sort_terms()
        self.indexer.add_skip_connections()
        self.indexer.calculate_tf_idf(tfdict)
    ...
    def run_queries(self, query_list, random_command):
        ...
        for query in tqdm(query_list):
            ...
            input_term_arr = self.preprocessor.tokenize_query(query)  # Tokenized query. To be implemented.
            ...
            # print(input_term_arr)
            and_op_no_skip, and_comparisons_no_skip, and_op_skip, and_comparisons_skip, and_op_no_skip_sorted, and_comparisons_no_skip_sorted, and_op_skip_sorted, and_comparisons_skip_sorted = self._daat_and(input_term_arr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--username", type=str,
                        help="Your UB username. It's the part of your UB email id before the @buffalo.edu. "
                             "DO NOT pass incorrect value here", default="nandaku3")
